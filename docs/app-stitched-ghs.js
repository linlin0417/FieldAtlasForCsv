...
const THEME_STORAGE_KEY = 'fieldAtlas.theme.stitched';
let keywordInput;
let resultList;
let resultSummary;
let modalDialog;
...
function applyTheme(theme) {
  ...
  themeToggleButton.textContent = isDark ? '切換淺色' : '切換暗色';
}
...
function createResultGhsPreview(item) {
  const wrapper = document.createElement('div');
  wrapper.className = 'result-ghs result-ghs-strip';
  ...
  const maxVisible = 4;
  const merged = createMergedGhsView(ghsList.slice(0, maxVisible), { mini: true });
  if (!merged) {
    const empty = document.createElement('span');
    empty.className = 'result-ghs-empty';
    empty.textContent = '無可用 GHS 圖示';
    wrapper.appendChild(empty);
    return wrapper;
  }

  wrapper.appendChild(merged);
...
function createMergedGhsView(list, { mini = false } = {}) {
  const normalizedCodes = normalizeGhsCodes(list);
  if (!normalizedCodes.length) {
    return null;
  }

  const shell = document.createElement('div');
  shell.className = mini ? 'ghs-merged ghs-merged--mini is-loading' : 'ghs-merged is-loading';
  shell.style.setProperty('--ghs-count', String(normalizedCodes.length));

  const image = document.createElement('img');
  image.className = 'ghs-merged-image';
  image.alt = `${normalizedCodes.join(' + ')} 合併圖示`;
  image.decoding = 'async';
  shell.appendChild(image);

  renderMergedGhsImage(image, normalizedCodes, { mini, shell });
  return shell;
}

function normalizeGhsCodes(list) {
  if (!Array.isArray(list)) {
    return [];
  }
  return list.map(code => code?.toString().trim()).filter(Boolean);
}

function renderMergedGhsImage(targetImage, ghsCodes, { mini = false, shell } = {}) {
  getMergedGhsImageDataUrl(ghsCodes, { mini })
    .then(dataUrl => {
      if (!dataUrl) {
        markMergedGhsAsFailed(shell, '無可用 GHS 圖示');
        return;
      }
    ...
    .catch(error => {
      console.warn('GHS 合併圖示失敗:', error);
      markMergedGhsAsFailed(shell, 'GHS 合圖失敗');
    });
}

function markMergedGhsAsFailed(shell, message) {
  if (!shell) {
    return;
  }
  shell.classList.remove('is-loading');
  shell.classList.add('ghs-merged--empty');
  shell.textContent = message;
}

function getMergedGhsImageDataUrl(ghsCodes, { mini = false } = {}) {
  const normalizedCodes = normalizeGhsCodes(ghsCodes);
  if (!normalizedCodes.length) {
    return Promise.resolve(null);
  }

  const cacheKey = `${mini ? 'mini' : 'full'}|${normalizedCodes.join(',')}`;
  const cachedTask = state.mergedGhsCache.get(cacheKey);
  if (cachedTask) {
    return cachedTask;
  }

  const task = buildMergedGhsImageDataUrl(normalizedCodes, { mini })
    .catch(error => {
      state.mergedGhsCache.delete(cacheKey);
      throw error;
    });

  state.mergedGhsCache.set(cacheKey, task);
  return task;
}

async function buildMergedGhsImageDataUrl(ghsCodes, { mini = false } = {}) {
  const fileNames = ghsCodes
    .map(resolveGhsIconFileName)
    .filter(Boolean);
  ...
  const iconCanvasSize = mini ? 56 : 72;
  const imageJobs = fileNames.map(fileName => loadGhsImageElement(fileName));
  const loadedImages = await Promise.all(imageJobs);

  const canvas = document.createElement('canvas');
  canvas.width = iconCanvasSize * loadedImages.length;
  canvas.height = iconCanvasSize;
  ...
  loadedImages.forEach((image, index) => {
    const offsetX = index * iconCanvasSize;
    ctx.drawImage(image, offsetX, 0, iconCanvasSize, iconCanvasSize);
  });

  return canvas.toDataURL('image/png');
}

function loadGhsImageElement(fileName) {
  const url = `${GHS_ICON_ROOT}/${encodeURIComponent(fileName)}`;
  return new Promise((resolve, reject) => {
    const image = new Image();
    image.decoding = 'async';
    image.onload = () => resolve(image);
    image.onerror = () => reject(new Error(`無法載入 GHS 圖示: ${fileName}`));
    image.src = url;
  });
}
...
function renderGhsIcons(list) {
  ...
  const merged = createMergedGhsView(list);
  if (!merged) {
    detailGhs.textContent = '—';
    return;
  }

  detailGhs.appendChild(merged);
}
...
async function renderMarkdown(casNo) {
  ...
  try {
    ...
    const sanitizedMarkdown = stripGhsCodesFromMarkdown(markdown);
    state.lastMarkdownSource = sanitizedMarkdown;
    const html = transformMarkdown(sanitizedMarkdown);
    markdownContent.innerHTML = html || '<p>無 Markdown 內容。</p>';
    copyMarkdownButton.disabled = !sanitizedMarkdown.trim();
  } catch (error) {
...
function stripGhsCodesFromMarkdown(markdown) {
  const text = markdown.replace(/^\uFEFF/, '');
  const lines = text.split(/\r?\n/);
  const output = [];
  let skipGhsBlock = false;
  let ghsBlockIndent = 0;

  lines.forEach(line => {
    const trimmed = line.trim();
    const indent = line.length - line.trimStart().length;

    if (!skipGhsBlock && isGhsHeaderLine(trimmed)) {
      skipGhsBlock = true;
      ghsBlockIndent = indent;
      return;
    }
    ...
    if (skipGhsBlock) {
      ...
      if (isGhsCodeLine(trimmed)) {
        return;
      }

      if (indent > ghsBlockIndent) {
        return;
      }
...
function isGhsHeaderLine(trimmedLine) {
  return /^-?\s*GHS\s*[:：]\s*$/i.test(trimmedLine);
}

function isGhsCodeLine(trimmedLine) {
  const normalized = trimmedLine.replace(/^-\s*/, '').trim();
  if (!normalized) {
    return false;
  }

  return /^(?:GHS\d{2}|無資料)(?:\s*(?:,|\/|、)\s*(?:GHS\d{2}|無資料))*$/i.test(normalized);
}
