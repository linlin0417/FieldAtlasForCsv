...
## 編輯介面
執行 `npm run edit` 後，會啟動專用的編輯頁面（預設走訪 http://127.0.0.1:5004/ ）。
該介面可以視覺化填寫或修改每筆化學物質資訊，包含分子量、GHS 圖示、LD50/LC50 與急救措施。
畫面會自動顯示 `data/ghs` 資料夾內的 GHS 圖示供勾選，並透過新的 `/api/ghs` 端點提供清單。

## 資料欄位更新
所有 SDS 條目現在在原有欄位外會額外紀錄：

- `MolecularWeight`：位於化學式下方，可用於標示常見分子量資訊。  
- `GHS`：放在危害分類區塊上方，代表對應的 GHS 圖示代碼，前端也會在查詢頁面額外渲染這些圖像。  
- `LC50`：置於 `LD50` 下方，方便紀錄急性吸入/皮膚暴露資料。
